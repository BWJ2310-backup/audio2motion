...
HOST = os.getenv("ECHOAVATAR_HOST", "0.0.0.0")
...
class StreamCoordinateConverter:
    # Converts BVH rotation and translation data into the stream coordinate format.
    def __init__(
        self,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Streaming vLLM server for body/face motion inference."
    )
    parser.add_argument(
        "--model_name",
        "--model-name",
...
if __name__ == "__main__":
    ...
    def setup_motion_client(endpoint):
        """Establish a persistent connection to one motion output receiver."""
        try:
            motion_client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            motion_client.connect((endpoint["host"], endpoint["port"]))
            print(
                f"Connected to motion receiver {endpoint['name']} "
                f"{endpoint['host']}:{endpoint['port']}"
            )
            return {"endpoint": endpoint, "sock": motion_client}
        except Exception as e:
            print(
                f"Failed to connect to motion receiver {endpoint['name']} "
                f"{endpoint['host']}:{endpoint['port']}: {e}"
            )
            return None
    ...
    def close_motion_clients(clients):
    ...
    stream_converter = StreamCoordinateConverter()

    smoother = BlockQuaternionSmoother(window_length=5, poly_order=2, 
                                        boundary_blend_frames=3, upsample=True)


    def build_motion_packet(motion_axis_angle, trans, expression, audio):
        """
        Build the neutral EchoAvatar motion packet once before fanning it out.
        """ 
        try:
            time1 = time.time()
            motion_axis_angle,trans = stream_converter.convert(motion_axis_angle,trans)
            time2 = time.time()
            print(f"stream_converter time: {time2-time1:.4f}s")
            ...
            return message_header, message_bytes
                
        except Exception as e:
            print(f"Error while building motion data: {e}")
            raise

    def send_motion_packet(client, message_header, message_bytes):
        endpoint = client["endpoint"]
        sock = client.get("sock")
        if sock is None:
            refreshed = setup_motion_client(endpoint)
            if refreshed is None:
                return False
            client["sock"] = refreshed["sock"]
            sock = client["sock"]

        try:
            with _prof("sender.socket.sendall", extra=f"{endpoint['name']} bytes={len(message_bytes)}"):
                sock.sendall(message_header)
                sock.sendall(message_bytes)
            return True
        except socket.error as e:
            print(f"Error while sending to {endpoint['name']}: {e}")
            try:
                sock.close()
            except Exception:
                pass
        ...
        client["sock"] = refreshed["sock"]
        try:
            client["sock"].sendall(message_header)
            client["sock"].sendall(message_bytes)
            return True
        except socket.error as e:
            print(f"Retry failed while sending to {endpoint['name']}: {e}")
            try:
                client["sock"].close()
            except Exception:
                pass
    ...
    def send_motion_data(clients, motion_axis_angle, trans, expression, audio):
        message_header, message_bytes = build_motion_packet(
            motion_axis_angle, trans, expression, audio
        )
        ok = True
        for client in clients:
            ok = send_motion_packet(client, message_header, message_bytes) and ok
        return ok
    ...
    def body_worker():
        ...
        audio_context_local: list[float] = []
        last_t = time.time()
        ...
        while True:
            ...
            # Maintain 4 seconds of rolling audio context for EnCodec token extraction.
            audio_context_local.extend(audio_chunk_24k.tolist())
            max_hist = int(rate * 4)
            if len(audio_context_local) > max_hist:
                audio_context_local = audio_context_local[-max_hist:]
            time3 = time.time()
            print(f"extend context time: {time3-time2:.4f}s")
            # EnCodec tokens: take tokens for the last 0.4 seconds of context and append them to hubert_codes.
            wav_2d = np.asarray(audio_context_local, dtype=np.float32).reshape(-1, 1)
    ...
    def face_worker():
        ...
        while True:
            ...
            face_context_local.extend(audio_chunk_24k.tolist())
            max_hist = int(FACE_AUDIO_RATE_RECV * 10)  # 10 seconds
            if len(face_context_local) > max_hist:
                face_context_local = face_context_local[-max_hist:]

            # Take 63 context frames (zero-pad if insufficient)
            if len(face_context_local) >= context_audio_len_24k:
                audio_for_infer_24k = np.asarray(face_context_local[-context_audio_len_24k:], dtype=np.float32)
            else:
                audio_for_infer_24k = np.zeros((context_audio_len_24k,), dtype=np.float32)
                context_arr = np.asarray(face_context_local, dtype=np.float32)
                audio_for_infer_24k[-len(context_arr):] = context_arr
    ...
    def sender_worker():
        ...
        while True:
                ...
                if done == {"body", "face"}:
                    if started and next_id is not None:
                        while next_id in body_buf and next_id in face_buf:
                            ...
                            clients = motion_client_box["clients"]
                            if clients:
                                try:
                                    send_motion_data(
                                        clients,
            ...
            while started and next_id in body_buf and next_id in face_buf:
                ...
                clients = motion_client_box["clients"]
                if not clients:
                    print("[sender] no motion receivers are connected; skipping send")
                    next_id += 1
                    continue
                ...
                if not ok:
                    print("[sender] one or more motion receivers failed this packet")
    ...
    # Establish persistent connections to configured motion receivers.
    motion_client_box["clients"] = setup_motion_clients()
    if not motion_client_box["clients"]:
        print("Unable to connect to any motion receiver; exiting")
        server_socket.close()
        exit(1)
    ...
    try:
        ...
        # Close motion receiver connections
        try:
            close_motion_clients(motion_client_box["clients"])
        except Exception:
            pass
